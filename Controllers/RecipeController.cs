...
namespace HeroServer.Controllers
{
    ...
    public class RecipeController : Controller
    {
        ...
        [HttpGet]
        public async Task<ActionResult<RecipeFull>> GetFullById([FromQuery] String id)
        {
            try
            {
                return Ok(await RecipeFunctions.GetFullById(Convert.ToInt64(id)));
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<TaleFull>> GetFullByPostId([FromQuery] String postId)
        {
            try
            {
                return Ok(await RecipeFunctions.GetFullByPostId(Convert.ToInt64(postId)));
            }
            catch (Exception ex)
            {
                return BadRequest(ex.Message);
