...
namespace HeroServer.Controllers
{
    ...
    public class TaleController : Controller
    {
        ...
        [HttpGet]
        public async Task<ActionResult<TaleFull>> GetFullById([FromQuery] String id)
        {
            try
            {
                return Ok(await TaleFunctions.GetFullById(Convert.ToInt64(id)));
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<TaleFull>> GetFullByPostId([FromQuery] String postId)
        {
            try
            {
                return Ok(await TaleFunctions.GetFullByPostId(Convert.ToInt64(postId)));
            }
            catch (Exception ex)
            {
                return BadRequest(ex.Message);
        ...
        public async Task<ActionResult<bool>> Accept([FromBody] PostModerationRequest postModerationRequest)
        {
            try
            {
                return Ok(await TaleFunctions.Accept(postModerationRequest.PostId, postModerationRequest.Id));
            }
            catch (Exception ex)
            {
                return BadRequest(ex.Message);
        ...
        public async Task<ActionResult<bool>> Reject([FromBody] PostModerationRequest postModerationRequest)
        {
            try
            {
                return Ok(await TaleFunctions.Reject(postModerationRequest.PostId, postModerationRequest.Id));
            }
            catch (Exception ex)
            {
                return BadRequest(ex.Message);
