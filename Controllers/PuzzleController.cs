...
namespace HeroServer.Controllers
{
    ...
    public class PuzzleController : Controller
    {
        ...
        [HttpGet]
        public async Task<ActionResult<PuzzleFull>> GetFullById([FromQuery] String id)
        {
            try
            {
                return Ok(await PuzzleFunctions.GetFullById(Convert.ToInt64(id)));
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<TaleFull>> GetFullByPostId([FromQuery] String postId)
        {
            try
            {
                return Ok(await PuzzleFunctions.GetFullByPostId(Convert.ToInt64(postId)));
            }
            catch (Exception ex)
            {
                return BadRequest(ex.Message);
