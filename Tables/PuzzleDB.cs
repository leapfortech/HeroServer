    ...
    public class PuzzleDB
    {
        ...
        public async Task<PuzzleFull> GetFullById(long id)
        {
            String strCmd = $"SELECT {table}.Id, {table}.PostId," +
                            ...
                            $" {table}.PuzzleTypeId, {table}.PuzzleSubtypeId, {table}.Question, {table}.Hint," +
            ...
            strCmd += "SELECT Comment.Id, Comment.AppUserId, AppUser.Alias AS AppUserAlias," +
                      " Comment.Comment, Comment.UpdateDateTime, Comment.Status" +
                      " FROM [D-PuzzleComment] AS Comment" +
                      " LEFT JOIN [D-AppUser] AS AppUser ON (Comment.AppUserId = AppUser.Id)" +
                      " WHERE Comment.Status = 1 AND Comment.PuzzleId = @Id;";
            ...
            using (conn)
            {
                ...
                using (SqlDataReader reader = await command.ExecuteReaderAsync())
                {
                    ...
                    await reader.NextResultAsync();
                    puzzleFull.PuzzleCommentFulls = [];
                    while (await reader.ReadAsync())
                        puzzleFull.PuzzleCommentFulls.Add(PuzzleCommentDB.GetPuzzleCommentFull(reader));
                }
            }
        ...
        public async Task<PuzzleFull> GetFullByPostId(long postId)
        {
            ...
            strCmd += "SELECT Comment.Id, Comment.AppUserId, AppUser.Alias AS AppUserAlias," +
                      ...
                      " FROM [D-PuzzleComment] AS Comment" +
                      " LEFT JOIN [D-AppUser] AS AppUser ON (Comment.AppUserId = AppUser.Id)" +
                      " WHERE Comment.Status = 1 AND Comment.PuzzleId IN" +
                      $" (SELECT Id FROM {table} WHERE PostId = @PostId);";
        ...
        public async Task<PuzzleDataFull> GetFullsByStatus(int status)
        {
            ...
            strCmd +=  "SELECT Comment.Id, Comment.PuzzleId, Comment.AppUserId, AppUser.Alias AS AppUserAlias," +
                       " Comment.Comment, Comment.UpdateDateTime, Comment.Status" +
                       " FROM [D-PuzzleComment] AS Comment" +
                      $" JOIN {table} ON (Comment.PuzzleId = {table}.Id)" +
                       " JOIN [D-AppUser] AS AppUser ON (Comment.AppUserId = AppUser.Id)" +
            ...
            using (conn)
            {
                ...
                using (SqlDataReader reader = await command.ExecuteReaderAsync())
                {
                    ...
                    while (await reader.ReadAsync())
                        puzzleCommentFulls.Add(PuzzleCommentDB.GetPuzzleCommentFull(reader));
                    puzzleDataFull.PuzzleCommentFulls = puzzleCommentFulls;
                }
            }
